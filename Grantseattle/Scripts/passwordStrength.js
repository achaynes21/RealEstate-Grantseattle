...
function passwordStrengthChecker(el) {
	/*var pwdStrengthIndicatorWidth = function() {
		jQuery('#pnlPasswStrengthIndicator').parent().css({
			'width': el.outerWidth(),
			'height': '5px',
	...
	indicatorHTML = '<div class="password-strength-container noTransition" style="height:5px; margin-top: 3px;">' +
						'	<div id="pnlPasswStrengthIndicator" style="height: 100%; background-color: #dddddd; width: 100% !important;">' +
						'		<div id="pnlPasswIndicator" style="height: 100%;">' +
						'			&nbsp;' +
						'		</div>' +
						'	</div>' +
						'	<label id="lblEvaluation" style="width: auto!important; font-size: 12px;padding:0;"></label>' +
						'</div>';

	el.after(indicatorHTML);
	...
	var ShowStrength = function (el, minLength, preferedLength, minNonAlphaNumChars, preferedNonAlphaNumChars, regularExpression, passwordLabelID, policyStrings, classPrefix, usePolicy, indicatorPanelID, useStylesForStrenghtIndicator) {
		...
		if (el && labelElem) {
			...
			if (minLength) {
				if (passwordLength == 0) {
					labelElem.innerHTML = '';
					jQuery('#pnlPasswStrengthIndicator').parent().css('height', '5px');
					...
					jQuery('#pnlPasswStrengthIndicator').parent().css('height', 'auto');
					return;
				}
			}
	...
	jQuery(function () {
		...
		el.keyup(function (event) {
			ShowStrength(el.get(0), 6, 6, 0, 1, regex, "lblEvaluation", sb, "", false, "pnlPasswIndicator", "");
		});
	});
}
...
var PasswordStrengthChecker = (function (j) {
	return {
		...
		init: function (selector) {
			passwordStrengthChecker(j(selector));
		}
	}
})(jQuery);